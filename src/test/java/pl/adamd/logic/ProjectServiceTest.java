...
class ProjectServiceTest {
    ...
    //-------------------------------------------------------------------------------------------------------------
    @Test
    @DisplayName("should throw IllegalArgumentException when configured allow jut 1 group and no groups and no projects for a given id")
    ...
    void createGroup_configurationOk_existsProject_createsAndSavesGroup() {
        ...
        // and
        var project = projectWith("pub", Set.of(-1, -2));
        var mockRepository = mock(ProjectRepository.class);
        ...
        // then
        assertThat(result.getDescription()).isEqualTo("pub");
        assertThat(result.getDeadline()).isEqualTo(today.minusDays(1));
        assertThat(result.getTasks()).allMatch(task -> task.getDescription().equals("oop"));
    ...
    private Project projectWith(String projectDescription, Set<Integer> daysToDeadline) {
        ...
        Set<ProjectStep> steps = daysToDeadline.stream()
                .map(days -> {
                    var projectStep = mock(ProjectStep.class);
                    when(projectStep.getDescription()).thenReturn("oop");
                    when(projectStep.getDaysToDeadline()).thenReturn(days.longValue());
                    return projectStep;
                })
    ...
    private static class InMemoryGroupRepository implements TaskGroupRepository {
        ...
        public boolean existsByDoneIsFalseAndProject_Id(final Integer projectId) {
            return map.values()
                    .stream().filter(taskGroup -> !taskGroup.isDone())
                    .anyMatch(taskGroup ->
